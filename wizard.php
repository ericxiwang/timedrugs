<?php
include 'nav_menu_banner.php';

?>
    <!--nav end -->
   
<div class="col-md-9 col-lg-9 panel panel-danger product_list" >


<div class='panel panel-success' align="center"><font size=5>订购指南</font></div>
<img  src="image/order.gif" alt="First slide" style="width:70%">


<table  width="100%" border="0" cellspacing="15" cellpadding="0">
           
            <tr>
              <td>
			  <style>.wizard td{border:1px solid #cccccc;font-size:16px;height:30px;width="100px"}</style>
			  <div class="wizard">
				<br/>	<h3>注：
				        1、线下付款客户，请及时提供订单号码，以便客服人员查询订单信息。</h3>
						<br/>
			  <table  width="703" border="0" cellspacing="0" cellpadding="0">
			  <tbody>
				<tr >
					<td colspan="2" width="200px">配送方式</td>
					<td width="100px">配送时间</td>
					<td width="100px">运费</td>
				</tr>
				<tr>
					<td width="100px" rowspan="2">加拿大境内
					</td>
					<td width="100px">
					普通运送
					</td>
					<td width="100px">
					4-9个工作日
					</td>
					<td width="100px">
					结帐自动计算或电邮查询
					</td>
				</tr>
				<tr>
					
					<td>
					特快运送
					</td>
					<td>
					1-3个工作日
					</td>
					<td>
					结帐自动计算或电邮查询
					</td>
				</tr>
				
				<tr>
					<td>
					加拿大境外
					</td>
					<td>
					邮政或专递
					</td>
					<td>
					7-14个工作日
					</td>
					<td>
					结帐自动计算或电邮查询
					</td>
				</tr>
				</tbody>
				</table>
    
      
    </div>

<div>注：<br/>
1、配送时间为在清关正常的情况下从快递取件后的第二个工作日开始计算<br/>
2、其他地区请电邮查询，因圣诞节，中国春节期间，运送有可能延误。<br/>
3、因邮费和寄送达时间会变化，以当日邮件查询好时光报价为准。。<br/>
4、任何因收货国征收关税或扣押检查引起耽误，好时光将不承担由此带来的时间和经济上的损失。<br/><br/>

</div>
<hr>
<div>
支付方式<br/>
加拿大境内购买产品，欢迎光临以下实体店直接购买：<br/>
 
加拿大第一专卖店<br/>
丽晶广场1101-4500 Kingsway<br/>
Burnaby, BC<br/>
Canada, V5H 2A9<br/><br/>
 
加拿大第二专卖店<br/>
#2135 3700 No 3 Road (YaoHan Center 二楼）<br/>
Richmond, BC<br/>
Canada, V6X 3Z8<br/><br/>
 
 
北美免费电话：1.888.505.5676<br/>
电话： 1.604.871.0178，1.604.568.9518<br/>
传真： 1.604.871.0179<br/>
电子邮件：info@timedrugs.com<br/><br/>
</div>
<hr>
<div>
加拿大境外订单都是先款后货订单：<br/>
中国境内开户行号：<br/>
中国工商银行贵阳中华路支行，姓名：施仲伟 账号：955-880-240-210-197-1519<br/>
 
注意事项：<br/>
1、  办理电汇时，请在电汇单“汇款用途”一栏处注明您的订单号或姓名及产品名称。<br/>
2、  支持所有银行的网上银行转账。<br/>
3、  实时到款，好时光保健药房网上商城实时确认订单。<br/><br/><br/>
 
 
售后服务<br/>
您可以任何时候发送邮件至邮箱，我们将竭诚为您提供快捷便利的服务，我们的服务时间段为:<br/>
好时光保健药房网上商城客服邮箱: info@timedrugs.com<br/><br/>

<div class='panel panel-danger' id='info'>

<h5>用户信息保护</h5>
<h5>所有的客户信息都通过安全的服务器，信息在加密后才会传输给我们。信用卡信息是通过使用一个安全服务器（SSL安全套接字层加密，网上安全交易标准）
</h5>
</div>

<div class='panel panel-danger' id='privacy'>

<h5>用户隐私政策</h5>
<h5>Timedrugs.com尊重您的隐私权，并致力于保护它，您的信息不会被传递到第三方，除非您明确同意。
如果你还不确定是否在互联网上发送您的信用卡详细信息。您还可以将您的订单信息以及付款方式发送邮件至info@timedrugs
com，我们有客服人员为您服务。
</h5>
</div>
<div class='panel panel-danger' id='return'>

<h5>退货退款政策</h5>
<h5>收到货物7天之内通过电子邮件向我们提出退换/退款申请，合理原因，本公司尽力配合顾客需要，但以下情况除外：<br/>
1）来回邮寄费不能退还<br/>
2）因所在国海关的扣查、没收、关税、罚款、延误等不属于退款之列<br/>
3）请保持退回产品干净并包装完整未开封状态。联系邮件：info@timedrugs.com 。
</h5>
</div>



</div>











    
 </div>


</div>
<!--inventory row end-->
  
</div>


</body>

</html>